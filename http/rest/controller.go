...
func create(c *gin.Context) {
	...
	if userID == "" {
		restErr := &utils.RestErr{
			Message: "user_id is not specified.",
			Status:  400,
	...
	if err != nil {
		restErr := &utils.RestErr{
			Message: "can't operate creation.",
			Status:  500,
			Error:   err.Error(),
		}
		c.JSON(restErr.Status, restErr)
...
func refresh(c *gin.Context) {
	...
	if err != nil {
		restErr := &utils.RestErr{
			Message: "refresh_token cookie is not set.",
			Status:  400,
			Error:   "error: bad request",
		}
		c.JSON(restErr.Status, restErr)
	...
	if err != nil {
		restErr := &utils.RestErr{
			Message: "can't operate refreshing.",
			Status:  400,
			Error:   err.Error(),
		}
		c.JSON(restErr.Status, restErr)
...
func verify(c *gin.Context) {
	...
	if err != nil {
		restErr := &utils.RestErr{
			Message: "access_token cookie is not set.",
			Status:  400,
			Error:   "error: bad request",
		}
		c.JSON(restErr.Status, restErr)
...
func revoke(c *gin.Context) {
	...
	if err := s.Revoke(accessToken, refreshToken); err != nil {
		restErr := &utils.RestErr{
			Message: "can't operate revokation.",
			Status:  400,
			Error:   err.Error(),
		}
		c.JSON(restErr.Status, restErr)
	...
	c.JSON(http.StatusOK, gin.H{"message": "tokens are revoked"})
	return
}
